...
class HMM:
    ...
    def tag(self, observations):
        ...
        """
        tags = []
        # Prepare the viterbi and backpointer data structures
        new_viterbi = np.zeros((len(self.states), len(observations)))
        new_backpointer = np.zeros((len(self.states), len(observations)))
        new_viterbi[:, 0] = self.viterbi[:, 0]
        new_backpointer[:, 0] = self.backpointer[:, 0]
        self.viterbi = new_viterbi
        ...
        for t in range(0, len(observations) - 1):  # iterate over steps
            for s in range(0, len(self.states)):  # iterate over states
                min_suppose = 999999  # Suppose the min value
                min_index = 0  # For recording the number of the min
                for i in range(0, len(self.states)):  # loop states
                    new_vit = self.viterbi[i][t] - self.tlprob(self.states[i], self.states[s]) - self.elprob(self.states[s],observations[t + 1]) # get new viterbi
                    # Get the min
                    if new_vit < min_suppose:
                        min_suppose = new_vit
                        min_index = i

                self.viterbi[s][t + 1] = min_suppose # Update the witerbi and backpointer
        ...
        minimun = 999999
        tag_num = []
        for a in range(0, len(self.states)):
            vit_new = self.viterbi[a, len(observations) - 1] - self.tlprob(self.states[a], '</s>') # Add the </s> for judging whether a sentence is end or not
            # Get the min
            if vit_new < minimun:
                minimun = vit_new
                best_path = a
        tag_num.append(best_path)

        for b in range(0, len(observations) - 1):
            num = int(tag_num[b])
            obs_num = (len(observations) - b - 1)
            tag_num.append(self.backpointer[num][obs_num])

        for c in range(0, len(observations)):
            i = int(tag_num[len(observations) - c - 1])
            tags.append(self.states[i]) # Get the tag sequence for what estimate
...
def answer_question5():
        ...
        return inspect.cleandoc("""First, the bigram and trigram model can be used here because we can get best performances of POS tags for the given words, especially since we have 100% coverage on grammar constructions, the bigram model which is a sequence of two adjacent tokens will work better. Second, for those words which did not appear before, we can use models like CYK algorithm to train data and give out the most likely tags by estimating the relationship between grammars.""")[0:500]
...
def answer_question6():
        ...
        return inspect.cleandoc("""If we do not use universal tagset, we will use original tagset which contains much larger tags than those in universal tagset. Therefore, the accuracy will decrease and it will also cost more time and space to compute and store data. Moreover, it is because of the limited POS tags in universal tagset that we can ensure each tags has enough training data which can avoid no words or little words appear in some tags if using original tagset.""")[0:500]
